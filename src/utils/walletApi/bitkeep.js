...
async function queryState() {
  ...
  const addressHex = await tryToConvertAddressToHex(WalletName.Bitkeep, address);
...
async function init() {
  try {
    ...
    store.getters.getWallet(WalletName.Bitkeep).deferred.resolve();
  }
}
...
async function sendSelfPayTx({ data, toAddress, toChainId }) {
  try {
    ...
    const transactionParameters = {
      ...
      // gasPrice: `0x${reverseHex(integerToHex(1000000))}`, // customizable by user during Bitkeep confirmation.
      // gas: `0x${reverseHex(integerToHex(400000))}`, // customizable by user during Bitkeep confirmation.
