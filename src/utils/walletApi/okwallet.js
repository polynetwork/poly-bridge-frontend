...
const OK_WALLET_CONNECTED_KEY = 'OK_WALLET_CONNECTED';
...
async function queryState() {
  ...
  const addressHex = await tryToConvertAddressToHex(WalletName.OkWallet, address);
  const checksumAddress = address && web3.utils.toChecksumAddress(address);
  const network = await window.okxwallet.request({ method: 'eth_chainId' });
...
async function init() {
  try {
    ...
    store.dispatch('updateWallet', { name: WalletName.OkWallet, installed: true });

    if (sessionStorage.getItem(OK_WALLET_CONNECTED_KEY) === 'true') {
      await queryState();
    }
    ...
    window.okxwallet.on('chainChanged', network => {
      store.dispatch('updateWallet', {
        name: WalletName.OkWallet,
        chainId: NETWORK_CHAIN_ID_MAPS[Number(network)],
      });
    ...
    store.getters.getWallet(WalletName.OkWallet).deferred.resolve();
  }
}
...
async function connect() {
  try {
    ...
    await queryState();
    sessionStorage.setItem(OK_WALLET_CONNECTED_KEY, 'true');
  } catch (error) {
...
async function sendSelfPayTx({ data, toAddress, toChainId }) {
  try {
    ...
    const transactionParameters = {
      nonce: '0x00', // ignored by OkWallet
      // gasPrice: `0x${reverseHex(integerToHex(1000000))}`, // customizable by user during OkWallet confirmation.
      // gas: `0x${reverseHex(integerToHex(400000))}`, // customizable by user during OkWallet confirmation.
      ...
      chainId: `0x${reverseHex(integerToHex(toEthChainID))}`, // Used to prevent transaction reuse across blockchains. Auto-filled by OkWallet.
    };
